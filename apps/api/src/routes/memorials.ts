...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: memorials.map((m) => formatMemorial(m as unknown as Record<string, unknown>)),
    });
  } catch (err) {
...
router.patch('/:id', authenticate, async (req, res) => {
  try {
    ...
    const memorial = await prisma.memorial.update({
      ...
      data: {
        ...rest,
        biography: biography as Prisma.InputJsonValue | undefined,
        programme: programme as Prisma.InputJsonValue | undefined,
        announcements: announcements as Prisma.InputJsonValue | undefined,
